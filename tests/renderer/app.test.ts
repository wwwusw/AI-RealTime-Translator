...
describe('App', () => {
  it('renders the file workspace and primary interface in Chinese', () => {
    ...
    expect(html).toContain('系统声音实时字幕翻译')
    expect(html).toContain('系统声音模式已就绪')
    expect(html).toContain('开始采集系统声音')
  ...
  it('shows only the approved runtime settings with Chinese labels', () => {
    ...
    // Hidden settings: 字幕块时长, 源语言, 目标语言
    expect(html).not.toContain('字幕块时长')
    expect(html).not.toContain('源语言')
    expect(html).not.toContain('目标语言')
